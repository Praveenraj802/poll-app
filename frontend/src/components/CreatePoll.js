...
import { Plus, Trash2, HelpCircle, Save, ArrowLeft, Type } from 'lucide-react';
...
const CreatePoll = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            await api.post('/polls/create', { question, options });
            navigate('/');
        } catch (err) {
            console.error(err);
                            ...
                            {options.map((option, index) => (
                                ...
                                >
                                    <div className="relative flex-1">
                                        <div className="absolute left-4 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 text-xs font-bold flex items-center justify-center">
                                            {index + 1}
                                        </div>
                                        <input
                                            type="text"
                                            required
                                            className="w-full pl-12 pr-4 py-4 bg-gray-50 dark:bg-gray-700/50 border border-transparent focus:border-blue-500 focus:bg-white dark:focus:bg-gray-700 rounded-xl outline-none transition-all dark:text-white font-medium"
                                            value={option}
                                    ...
                                    {options.length > 2 && (
                                        <button
                                            ...
                                            className="p-4 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-all"
                                        >
                                            <Trash2 size={20} />
                                        </button>
                                    )}
